import { UMKMCard } from "@/type/type";

// shopee, tokopedia, tiktokshop, facebook

export const dataUMKMUnggulan: UMKMCard[] = [
  {
    id: 1,
    title: "DR Snack",
    image: "/img/mojodisplay.webp",
    alt: "DR Snack",
    kelurahan: "Gebang",
    summary: `DR Snack merupakan UMKM milik Bu Marsini, warga RT 18 Padukuhan Gebang, yang menghadirkan berbagai makanan olahan dari bahan lokal seperti singkong dan kacang tolo. Produk unggulannya meliputi brownies singkong, donat singkong, hingga bola-bola singkong keju yang nikmat dan inovatif. Dengan mengangkat potensi pangan lokal, DR Snack tidak hanya mendukung ketahanan pangan desa, tetapi juga membuka peluang ekonomi kreatif bagi warga. Cocok sebagai oleh-oleh khas atau sajian saat bersantai, DR Snack menjadi salah satu kebanggaan kuliner dari Desa Gebang.`,
    description: `Batik Mojo adalah salah satu jenis batik khas yang berasal dari  Kalurahan Jetis, Kecamatan Saptosari, Kabupaten Gunungkidul, Daerah Istimewa Yogyakarta. Batik ini memiliki kekhasan tersendiri yang membedakannya dari batik-batik daerah lain, baik dari segi motif, warna, maupun filosofi yang terkandung di dalamnya. 
      
              Motif batik Mojo banyak terinspirasi dari alam sekitar, seperti daun mojo, batuan karst, flora-fauna lokal, dan budaya masyarakat setempat. Warna yang digunakan umumnya berasal dari bahan-bahan alami seperti daun jati atau kulit kayu, sehingga menghasilkan warna-warna yang lembut dan alami. Proses pembuatannya masih dilakukan secara tradisional, terutama dengan teknik batik tulis dan cap manual, yang menjadikan setiap helai kain batik Mojo unik dan bernilai tinggi.
              `,
    images: [
      "/img/mojodisplay.webp",
      "/img/mojo2.webp",
      "/img/mojo3.webp",
      "/img/mojo4.webp",
    ],
    latitude: -8.036085882573504,
    longitude: 110.4851617521728,
    rangeHarga: "Rp10.000-Rp100.000",
    googleMapsLink: "https://maps.app.goo.gl/iUoCxoXiEwxrLX3aA",
    kontak: {
      namaUsaha: "Batik Mojo Bu Siti",
      namaPemilik: "Ibu Siti",
      alamat: "Jl. Batik Mojo No. 1, Jetis, Gunungkidul, Yogyakarta",
      noTelp: "081234567890",
      email: "asd@gmail.com",
      mediaSosial: [
        {
          nama: "@batik.mojo.siti",
          link: "https://www.instagram.com/batik.mojo.siti",
          platform: "instagram",
        },
        {
          nama: "@batikmojo",
          link: "https://www.tiktok.com/@batikmojo",
          platform: "tiktok",
        },
      ],
      onlineShop: [
        {
          namaToko: "Batik Mojo Bu Siti",
          link: "https://www.shopee.com",
          platform: "shopee",
        },
        {
          namaToko: "Batik Mojo Bu Siti",
          link: "https://www.tokopedia.com",
          platform: "tokopedia",
        },
        {
          namaToko: "Batik Mojo Bu Siti",
          link: "https://www.tiktok.com/shop",
          platform: "tiktokshop",
        },
      ],
    },
    kategoriUsaha: "Kerajinan",
    products: [
      {
        nama: "Batik Tulis Mojo",
        deskripsi: "Batik tulis dengan motif khas alam Gunungkidul.",
        images: ["/img/mojodisplay.webp", "/img/mojo2.webp", "/img/mojo3.webp"],
        harga: "Rp 150.000",
        labels: ["Batik Tulis", "Motif Alam", "Tradisional"],
      },
      {
        nama: "Batik Cap Mojo",
        deskripsi: "Batik cap dengan desain modern namun tetap tradisional.",
        images: ["/img/mojo2.webp"],
        harga: "Rp 100.000",
        labels: ["Batik Cap", "Desain Modern", "Tradisional"],
      },
    ],
  },
  {
    id: 2,
    title: "DR Snack",
    image: "/img/marsini.jpeg",
    alt: "DR Snack",
    kelurahan: "Gebang",
    summary: `DR Snack merupakan UMKM milik Bu Marsini, warga RT 18 Padukuhan Gebang, yang menghadirkan berbagai makanan olahan dari bahan lokal seperti singkong dan kacang tolo. Produk unggulannya meliputi brownies singkong, donat singkong, hingga bola-bola singkong keju yang nikmat dan inovatif. Dengan mengangkat potensi pangan lokal, DR Snack tidak hanya mendukung ketahanan pangan desa, tetapi juga membuka peluang ekonomi kreatif bagi warga. Cocok sebagai oleh-oleh khas atau sajian saat bersantai, DR Snack menjadi salah satu kebanggaan kuliner dari Desa Gebang.`,
    description: `Batik Mojo adalah salah satu jenis batik khas yang berasal dari  Kalurahan Jetis, Kecamatan Saptosari, Kabupaten Gunungkidul, Daerah Istimewa Yogyakarta. Batik ini memiliki kekhasan tersendiri yang membedakannya dari batik-batik daerah lain, baik dari segi motif, warna, maupun filosofi yang terkandung di dalamnya. 
      
              Motif batik Mojo banyak terinspirasi dari alam sekitar, seperti daun mojo, batuan karst, flora-fauna lokal, dan budaya masyarakat setempat. Warna yang digunakan umumnya berasal dari bahan-bahan alami seperti daun jati atau kulit kayu, sehingga menghasilkan warna-warna yang lembut dan alami. Proses pembuatannya masih dilakukan secara tradisional, terutama dengan teknik batik tulis dan cap manual, yang menjadikan setiap helai kain batik Mojo unik dan bernilai tinggi.
              `,
    images: [
      "/img/bakpia.jpeg",
      "/img/brownis-singkong.jpeg",
      "/img/kue-bundar2.jpeg",
      "/img/kue-keju.jpeg",
    ],
    latitude: -8.060680347649244,
    longitude: 110.49309062176623,
    rangeHarga: "Rp1.000-Rp100.000",
    googleMapsLink: "https://maps.app.goo.gl/QfxhVNEvjTRMXiTCA",
    kontak: {
      namaUsaha: "DR Snack",
      namaPemilik: "Ibu Marsini",
      alamat:
        "JL Sidowayah, Gebang, Ngloro, Kec. Saptosari, Kabupaten Gunungkidul, Daerah Istimewa Yogyakarta 55871",
      noTelp: "081280545709",
      email: "",
      mediaSosial: [],
      onlineShop: [],
    },
    kategoriUsaha: "Makanan",
    products: [
      {
        nama: "Bakpia",
        deskripsi:
          "Bakpia adalah salah satu produk unggulan DR Snack yang menjadi favorit banyak pelanggan. Makanan khas ini berbentuk bulat pipih dengan tekstur kulit lembut dan isian manis di dalamnya, seperti kacang hijau, cokelat, atau keju. Diproduksi secara higienis dan dikemas rapi, bakpia sangat cocok sebagai camilan keluarga maupun oleh-oleh khas. ",
        images: ["/img/bakpia.jpeg"],
        harga: "Rp 100.000",
        labels: ["Oleh-Oleh", "Camilan", "Kuliner Lokal"],
      },
      {
        nama: "Brownis Singkong",
        deskripsi: `Brownis Singkong merupakan salah satu produk andalan dari UMKM "DR Snack" yang menghadirkan cita rasa unik dari olahan bahan lokal. Dibuat dari singkong pilihan yang diolah menjadi kue brownis lembut dan legit, produk ini menjadi alternatif sehat tanpa mengurangi kenikmatan rasa. Dengan sentuhan cokelat yang kaya dan aroma khas singkong, Brownis Singkong cocok dinikmati sebagai camilan harian maupun suguhan spesial. `,
        images: ["/img/brownis-singkong.jpeg"],
        harga: "Rp 100.000",
        labels: ["Camilan", "Bahan Lokal", "Kuliner Lokal"],
      },
      {
        nama: "Kue Bolu Singkong Keju",
        deskripsi: `Kue Bolu Singkong Keju dari DR Snack merupakan perpaduan sempurna antara cita rasa tradisional dan sentuhan modern. Terbuat dari singkong pilihan sebagai bahan utama, kue ini memiliki tekstur lembut dan aroma khas yang menggugah selera. Taburan keju di atasnya menambah rasa gurih yang lezat, menjadikannya camilan sehat dan nikmat untuk segala usia. `,
        images: [
          "/img/kue-keju.jpeg",
          "/img/kue-keju2.jpeg",
          "/img/kue-keju3.jpeg",
        ],
        harga: "Rp 80.000",
        labels: ["Camilan", "Bahan Lokal", "Kuliner Lokal"],
      },
      {
        nama: "Kue Tar Singkong",
        deskripsi: `Kue Tar Singkong dari DR Snack merupakan kreasi unik yang mengangkat bahan lokal menjadi sajian istimewa. Dibuat dari singkong pilihan yang diolah dengan resep khas, kue ini menghadirkan rasa manis yang lembut dengan tekstur yang moist dan padat. Topping krim dan hiasan menarik di atasnya menambah daya tarik visual sekaligus cita rasa. Cocok untuk hidangan di berbagai momen spesial, Kue Tar Singkong tidak hanya lezat tetapi juga menjadi alternatif sehat dari olahan tepung terigu.`,
        images: [
          "/img/kue-meesis-keju.jpeg",
          "/img/kue-mesis-keju.jpeg",
          "/img/kue-oreo.jpeg",
          "/img/kue-tar.jpeg",
          "/img/kue-tar2.jpeg",
        ],
        harga: "Rp 50.000",
        labels: ["Camilan", "Bahan Lokal", "Kuliner Lokal"],
      },
      {
        nama: "Lumpia",
        deskripsi: `Lumpia Isi Ayam dari DR Snack adalah camilan gurih yang menggugah selera dengan isian ayam berbumbu khas yang kaya rasa. Dibungkus dengan kulit lumpia yang renyah saat digoreng, setiap gigitan menghadirkan perpaduan tekstur dan cita rasa yang lezat. Cocok disajikan sebagai snack harian maupun pelengkap hidangan di berbagai acara, Lumpia Isi Ayam ini menjadi pilihan favorit bagi pencinta makanan ringan yang praktis dan menggoda.`,
        images: ["/img/lumpia.jpeg"],
        harga: "Rp 1.000",
        labels: ["Camilan", "Bahan Lokal", "Kuliner Lokal"],
      },
      {
        nama: "Molen",
        deskripsi: `Lumpia Isi Ayam dari DR Snack adalah camilan gurih yang menggugah selera dengan isian ayam berbumbu khas yang kaya rasa. Dibungkus dengan kulit lumpia yang renyah saat digoreng, setiap gigitan menghadirkan perpaduan tekstur dan cita rasa yang lezat. Cocok disajikan sebagai snack harian maupun pelengkap hidangan di berbagai acara, Lumpia Isi Ayam ini menjadi pilihan favorit bagi pencinta makanan ringan yang praktis dan menggoda.`,
        images: ["/img/molen.jpeg"],
        harga: "Rp 1.000",
        labels: ["Camilan", "Bahan Lokal", "Kuliner Lokal"],
      },
      {
        nama: "Onde-Onde",
        deskripsi: `Onde-onde dari DR Snack merupakan jajanan tradisional yang tetap digemari sepanjang masa. Terbuat dari tepung ketan yang dibentuk bulat dan diisi dengan kacang hijau manis, onde-onde ini dibalut dengan taburan wijen di bagian luar, lalu digoreng hingga renyah keemasan. Teksturnya yang kenyal berpadu sempurna dengan isian lembut dan gurih, menjadikannya camilan khas yang cocok dinikmati kapan saja. Onde-onde DR Snack adalah pilihan tepat untuk pecinta cita rasa nusantara yang autentik.`,
        images: ["/img/onde-onde.jpeg"],
        harga: "Rp 1.000",
        labels: ["Camilan", "Bahan Lokal", "Kuliner Lokal"],
      },
      {
        nama: "Putu Ayu",
        deskripsi: `Putu Ayu dari DR Snack adalah kue tradisional yang lembut dan manis dengan tampilan cantik berwarna hijau serta taburan kelapa parut di bagian atasnya. Terbuat dari campuran tepung, santan, dan daun pandan, kue ini memiliki aroma harum khas yang menggugah selera. Teksturnya yang empuk dan rasa gurih manis yang seimbang membuat Putu Ayu menjadi camilan favorit untuk segala usia. Cocok disajikan dalam berbagai acara, kue ini menjadi pilihan sempurna bagi pencinta kudapan tradisional berkualitas.`,
        images: ["/img/putu-ayu.jpeg"],
        harga: "Rp 1.000",
        labels: ["Camilan", "Bahan Lokal", "Kuliner Lokal"],
      },
      {
        nama: "Sosis Risol Singkong",
        deskripsi: `Sosis Risol Singkong dari DR Snack adalah camilan gurih yang menggoda selera dengan perpaduan unik antara sosis lezat dan kulit risol berbahan dasar singkong. Dibuat dari singkong pilihan yang diolah hingga menghasilkan tekstur renyah di luar dan lembut di dalam, risol ini menawarkan rasa nikmat yang pas untuk segala usia. Cocok sebagai bekal, sajian acara, atau teman santai, Sosis Risol Singkong menjadi pilihan tepat bagi pencinta camilan praktis dengan cita rasa khas olahan lokal.`,
        images: ["/img/risol-mayo.jpeg", "sosis-risol.jpeg"],
        harga: "Rp 1.000",
        labels: ["Camilan", "Bahan Lokal", "Kuliner Lokal"],
      },
    ],
  },
  {
    id: 3,
    title: "Lemper Benguk",
    image: "/img/lemper-benguk.jpg",
    alt: "Lemper Benguk",
    kelurahan: "Jetis",
    summary: `Lemper benguk asli Jetis, Gunungkidul adalah makanan khas yang terbuat dari koro benguk, sejenis kacang lokal yang diolah sebagai pengganti ketan dalam lemper tradisional. Kudapan unik ini berasal dari Kalurahan Jetis, Kecamatan Saptosari, dan menjadi simbol inovasi kuliner yang memanfaatkan hasil pertanian lokal. Dengan cita rasa gurih dan tekstur padat, lemper benguk kini dikembangkan sebagai ikon kuliner sekaligus sarana pemberdayaan ekonomi masyarakat setempat.`,
    description: `Lemper benguk adalah varian unik dari lemper tradisional yang berasal dari Kalurahan Jetis, Kecamatan Saptosari, Gunungkidul. Tidak seperti lemper pada umumnya yang menggunakan beras ketan, lemper ini menggunakan bahan utama berupa koro benguk, sejenis kacang lokal yang banyak tumbuh di wilayah tersebut. Koro benguk terlebih dahulu direndam selama beberapa hari untuk menghilangkan zat racunnya, lalu direbus hingga empuk, dihancurkan, dan dibentuk menjadi adonan padat. Adonan ini kemudian diberi isian berupa abon ayam atau sapi, dibungkus dengan daun pisang, dan dikukus hingga matang.

Makanan khas ini awalnya dikembangkan oleh ibu-ibu dari Padukuhan Dondong sebagai upaya memanfaatkan hasil pertanian lokal dan memperkaya khazanah kuliner daerah. Kini, lemper benguk diproduksi secara pre-order dengan kapasitas produksi mencapai ratusan potong per hari. Pengelolaan produksinya dilakukan oleh Badan Usaha Milik Kalurahan (BUMKal) Jetis bernama Sembulur, yang juga berperan dalam promosi lemper benguk sebagai ikon kuliner lokal serta sarana pemberdayaan ekonomi masyarakat.

Lemper benguk tidak hanya dikenal sebagai kudapan khas, tetapi juga menjadi bagian dari paket wisata edukasi yang ditawarkan di Jetis. Wisatawan dapat melihat langsung proses pembuatan lemper benguk dari tahap perendaman, pengolahan adonan, hingga proses pengukusan. Hal ini menjadikan lemper benguk sebagai simbol inovasi kuliner berbasis budaya dan kearifan lokal yang memperkuat identitas serta kebanggaan masyarakat Gunungkidul.
    `,
    images: [
      "/img/lemper-benguk.jpg",
      "/img/lemper-benguk-2.webp",
      "/img/lemper-benguk3.jpg",
    ],
    latitude: -8.031770018143167,
    longitude: 110.48096424787224,
    rangeHarga: "Rp10.000-Rp100.000",
    kontak: {
      namaUsaha: "Lemper Benguk Jetis",
      namaPemilik: "Ibu Sukamti",
      alamat: "Jl. Lemper Benguk No. 1, Jetis, Gunungkidul, Yogyakarta",
      noTelp: "081234567890",
      mediaSosial: [
        {
          nama: "@lemper.benguk.sukamti",
          link: "https://www.instagram.com/lemper.benguk.sukamti",
          platform: "instagram",
        },
        {
          nama: "@lemper.benguk.sukamti7",
          link: "https://www.tiktok.com/@lemper.benguk.sukamti7",
          platform: "tiktok",
        },
      ],
      onlineShop: [
        {
          namaToko: "Batik Mojo Bu Siti",
          link: "https://www.shopee.com",
          platform: "shopee",
        },
      ],
    },
    kategoriUsaha: "Kuliner",
    products: [
      {
        nama: "Lemper Benguk Isi Ayam",
        deskripsi: "Lemper benguk dengan isian abon ayam yang gurih.",
        images: ["/img/lemper-benguk.jpg"],
        harga: "Rp 15.000",
        labels: ["Kuliner Lokal", "Tradisional", "Sehat"],
      },
      {
        nama: "Lemper Benguk Isi Sapi",
        deskripsi: "Lemper benguk dengan isian abon sapi yang lezat.",
        images: ["/img/lemper-benguk-2.webp"],
        harga: "Rp 15.000",
        labels: ["Kuliner Lokal", "Tradisional", "Sehat"],
      },
    ],
  },
  {
    id: 4,
    title: "Abon Lele",
    image: "/img/abon-lele.jpeg",
    alt: "Abon Lele",
    description: `Abon lele merupakan hasil olahan kreatif dari ikan lele yang dibudidayakan secara mandiri oleh masyarakat setempat. Produk ini lahir dari upaya pemberdayaan ekonomi keluarga, terutama melalui kegiatan ibu-ibu PKK dan kelompok usaha mikro yang memanfaatkan lele sebagai bahan baku utama. Dalam proses pembuatannya, ikan lele yang telah dipanen—biasanya berukuran besar—akan dikuliti, dikukus, lalu dagingnya dipisahkan dari tulang. Daging tersebut kemudian dibumbui dengan rempah-rempah tradisional seperti bawang putih, serai, lengkuas, dan daun jeruk, sebelum akhirnya ditumis dan digoreng kering hingga menjadi serat abon yang gurih dan tahan lama.

    Abon lele ini tidak hanya lezat, tetapi juga kaya nutrisi karena mengandung protein tinggi, omega-3, fosfor, kalsium, dan vitamin B12. Kandungan gizi tersebut menjadikan abon lele sebagai makanan sehat yang cocok untuk membantu pertumbuhan anak dan mencegah stunting. Produk ini juga menjadi solusi praktis dalam memperkenalkan konsumsi ikan dalam pola makan sehari-hari. Selain manfaat gizinya, abon lele Jetis juga berperan penting dalam mendukung ketahanan pangan lokal serta memberikan nilai tambah ekonomi bagi warga melalui pemasaran di pasar tradisional maupun secara daring. Inisiatif ini mencerminkan sinergi antara kearifan lokal dan inovasi pangan dalam meningkatkan kesejahteraan masyarakat.`,
    summary: `Abon lele adalah olahan kering dari daging ikan lele yang diproduksi secara mandiri oleh masyarakat setempat, terutama oleh kelompok ibu-ibu PKK dan UMKM. Produk ini dihasilkan melalui proses pengolahan tradisional yang memanfaatkan rempah-rempah alami, sehingga menghasilkan abon yang gurih, bergizi, dan tahan lama. Selain sebagai sumber pangan sehat, abon lele juga menjadi salah satu bentuk pemberdayaan ekonomi dan inovasi pangan lokal di wilayah Jetis.`,
    kelurahan: "Jetis",
    rangeHarga: "Rp10.000-Rp100.000",
    latitude: -8.039368294633421,
    longitude: 110.48893996869498,
    images: ["/img/abon-lele.jpeg"],
    kontak: {
      namaPemilik: "Dian Ramadhan	",
      namaUsaha: "Abon Lele Jetis",
      alamat: "Karang 03/06 jetis saptosari",
      noTelp: "085726666666",
      mediaSosial: [
        {
          nama: "@abon.lele7",
          link: "https://www.instagram.com/abon.lele7",
          platform: "instagram",
        },
        {
          nama: "@abon.lele8",
          link: "https://www.tiktok.com/@abon.lele8",
          platform: "tiktok",
        },
      ],
      onlineShop: [
        {
          namaToko: "Batik Mojo Bu Siti",
          link: "https://www.shopee.com",
          platform: "shopee",
        },
      ],
    },
    kategoriUsaha: "Kuliner",
    products: [
      {
        nama: "Abon Lele Original",
        deskripsi: "Abon lele dengan cita rasa gurih dan tekstur renyah.",
        images: ["/img/abon-lele.jpeg"],
        harga: "Rp 25.000",
        labels: ["Kuliner Lokal", "Sehat", "Praktis"],
      },
    ],
  },
  {
    id: 5,
    title: "Lempeng Thiwul",
    summary: `Lempeng thiwul Gunungkidul adalah camilan tradisional yang terbuat dari tepung gaplek, kelapa parut, dan gula merah, yang dibentuk pipih dan dipanggang. Makanan khas ini mencerminkan cita rasa lokal serta menjadi bagian dari warisan kuliner masyarakat Gunungkidul.`,
    description: `Lempeng thiwul adalah salah satu makanan tradisional khas Gunungkidul yang terbuat dari bahan dasar gaplek, yaitu singkong yang telah dikeringkan dan ditumbuk halus. Makanan ini berbentuk lempengan pipih dan bertekstur agak keras di luar namun empuk di dalam, dengan cita rasa manis gurih khas thiwul. Proses pembuatannya melibatkan pencampuran tepung gaplek dengan kelapa parut dan gula merah, lalu dibentuk pipih dan dipanggang hingga matang. Lempeng thiwul merupakan camilan yang cukup populer di kalangan masyarakat pedesaan Gunungkidul karena tahan lama, bergizi, dan memiliki rasa yang otentik, mencerminkan kearifan lokal serta kebiasaan pangan masyarakat setempat.`,
    image: "/img/lempeng-thiwul.jpeg",
    alt: "Lempeng Thiwul",
    latitude: -8.035355951771715,
    longitude: 110.48949996476786,
    kelurahan: "Ngloro",
    images: ["/img/lempeng-thiwul.jpeg"],
    rangeHarga: "Rp10.000-Rp100.000",
    kontak: {
      namaPemilik: "Ibu Tupon",
      namaUsaha: "Lempeng Thiwul Ngloro",
      alamat: "Jl. Lempeng Thiwul No. 1, Ngloro, Gunungkidul, Yogyakarta",
      noTelp: "081234567890",
      mediaSosial: [
        {
          nama: "@lempeng.thiwul8",
          link: "https://www.instagram.com/lempeng.thiwul7",
          platform: "instagram",
        },
        {
          nama: "@lempeng.thiwul8",
          link: "https://www.tiktok.com/@lempeng.thiwul8",
          platform: "tiktok",
        },
      ],
      onlineShop: [
        {
          namaToko: "Batik Mojo Bu Siti",
          link: "https://www.shopee.com",
          platform: "shopee",
        },
      ],
    },
    kategoriUsaha: "Kuliner",
    products: [
      {
        nama: "Lempeng Thiwul Original",
        deskripsi:
          "Lempeng thiwul dengan cita rasa manis gurih khas Gunungkidul.",
        images: ["/img/lempeng-thiwul.jpeg"],
        harga: "Rp 10.000",
        labels: ["Kuliner Tradisional", "Sehat", "Praktis"],
      },
      {
        nama: "Lempeng Thiwul Isi Kelapa",
        deskripsi: "Lempeng thiwul dengan isian kelapa parut yang lezat.",
        images: ["/img/lempeng-thiwul.jpeg"],
        harga: "Rp 12.000",
        labels: ["Kuliner Tradisional", "Sehat", "Praktis"],
      },
    ],
  },
  {
    id: 6,
    title: "Peyek",
    image: "/img/peyek.jpeg",
    summary: `Peyek Gunungkidul adalah camilan tradisional yang renyah dan gurih, dibuat dari tepung beras dengan tambahan kacang atau bahan lain seperti rebon, yang digoreng tipis dan kering. Makanan ini menjadi salah satu oleh-oleh khas dari daerah Gunungkidul, Yogyakarta, karena kelezatan dan keunikan rasanya yang khas.`,
    description: `Peyek Gunungkidul adalah camilan tradisional khas dari Kabupaten Gunungkidul, Daerah Istimewa Yogyakarta, yang terkenal dengan cita rasanya yang gurih dan renyah. Makanan ini dibuat dari adonan tepung beras yang dicampur dengan bumbu-bumbu khas, seperti bawang putih, ketumbar, dan garam, kemudian diberi tambahan kacang tanah, kedelai, atau rebon (udang kecil) di permukaannya. Setelah itu, adonan digoreng tipis hingga kering dan renyah. Ciri khas peyek dari Gunungkidul terletak pada teksturnya yang renyah dan rasa bumbunya yang kuat, sering kali dimasak dengan resep turun-temurun oleh masyarakat setempat. Peyek ini biasanya disajikan sebagai pelengkap hidangan utama seperti nasi, atau dinikmati sebagai camilan sehari-hari.`,
    latitude: -8.062865005585818,
    longitude: 110.49600616489971,
    alt: "Peyek",
    images: ["/img/peyek.jpeg"],
    kelurahan: "Ngloro",
    rangeHarga: "Rp10.000-Rp100.000",
    kontak: {
      namaPemilik: "Ibu Nurul",
      namaUsaha: "Peyek Ngloro",
      alamat: "Jl. Peyek Ngloro No. 1, Ngloro, Gunungkidul, Yogyakarta",
      noTelp: "081234567890",
      mediaSosial: [
        {
          nama: "@pheyek8",
          link: "https://www.instagram.com/pheyek7",
          platform: "instagram",
        },
        {
          nama: "@pheyek8",
          link: "https://www.tiktok.com/@pheyek8",
          platform: "tiktok",
        },
      ],
      onlineShop: [
        {
          namaToko: "Batik Mojo Bu Siti",
          link: "https://www.shopee.com",
          platform: "shopee",
        },
      ],
    },
    kategoriUsaha: "Kuliner",
    products: [
      {
        nama: "Peyek Kacang",
        deskripsi: "Peyek renyah dengan isian kacang tanah yang gurih.",
        images: ["/img/peyek.jpeg"],
        harga: "Rp 15.000",
        labels: ["Kuliner Tradisional", "Renyah", "Gurih"],
      },
    ],
  },
];
